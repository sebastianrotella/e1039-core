/*
KalmanFastTracking_OLD.cxx

Implementation of class Tracklet, KalmanFastTracking_OLD
...
KalmanFastTracking_OLD::KalmanFastTracking_OLD(const PHField* field, const TGeoManager* geom, bool flag): verbosity(0), enable_KF(flag), outputListIdx(4)
{
    using namespace std;
    initGlobalVariables();

#ifdef _DEBUG_ON
    cout << "Initialization of KalmanFastTracking_OLD ..." << endl;
    cout << "========================================" << endl;
#endif
...
KalmanFastTracking_OLD::~KalmanFastTracking_OLD()
{
...
void KalmanFastTracking_OLD::setRawEventDebug(SRawEvent* event_input)
{
    rawEvent = event_input;
    hitAll = event_input->getAllHits();
}

int KalmanFastTracking_OLD::setRawEvent(SRawEvent* event_input)
{
  totalTime = 0;
    ...
    for(int i = 0; i < 4; i++)
    {
        ...
        {
	  hitIDs_mask[i] = rawEvent->getHitsIndexInDetectors(detectorIDs_maskX[i]); //WPM
...
bool KalmanFastTracking_OLD::acceptEvent(SRawEvent* rawEvent)
{
...
void KalmanFastTracking_OLD::buildBackPartialTracks()
{
...
void KalmanFastTracking_OLD::buildGlobalTracks()
{
	...
	for(int i = 0; i < 1; ++i) //for two station-1 chambers //WPM
        {
...
void KalmanFastTracking_OLD::resolveLeftRight(Tracklet& tracklet, double threshold)
{
#ifdef _DEBUG_ON
...
void KalmanFastTracking_OLD::resolveSingleLeftRight(Tracklet& tracklet)
{
#ifdef _DEBUG_ON
...
void KalmanFastTracking_OLD::removeBadHits(Tracklet& tracklet)
{
#ifdef _DEBUG_ON
...
void KalmanFastTracking_OLD::resolveLeftRight(SRawEvent::hit_pair hpair, int& LR1, int& LR2)
{
    LR1 = 0;
    LR2 = 0;
...
void KalmanFastTracking_OLD::buildTrackletsInStation(int stationID, int listID, double* pos_exp, double* window)
{
#ifdef _DEBUG_ON
...
bool KalmanFastTracking_OLD::acceptTracklet(Tracklet& tracklet)
{
...
bool KalmanFastTracking_OLD::hodoMask(Tracklet& tracklet)
{
...
bool KalmanFastTracking_OLD::muonID_search(Tracklet& tracklet)
{
...
bool KalmanFastTracking_OLD::muonID_comp(Tracklet& tracklet)
{
    //Set the cut value on multiple scattering
    //multiple scattering: sigma = 0.0136*sqrt(L/L0)*(1. + 0.038*ln(L/L0))/P, L = 1m, L0 = 1.76cm
    double cut = 0.03;
...
bool KalmanFastTracking_OLD::muonID_hodoAid(Tracklet& tracklet)
{
    double win = 0.03;
    double factor = 5.;
    if(tracklet.stationID == nStations)
...
void KalmanFastTracking_OLD::buildPropSegments()
{
#ifdef _DEBUG_ON
...
int KalmanFastTracking_OLD::fitTracklet(Tracklet& tracklet)
{
...
int KalmanFastTracking_OLD::reduceTrackletList(std::list<Tracklet>& tracklets)
{
...
void KalmanFastTracking_OLD::getExtrapoWindowsInSt1(Tracklet& tracklet, double* pos_exp, double* window, int st1ID)
{
...
void KalmanFastTracking_OLD::getSagittaWindowsInSt1(Tracklet& tracklet, double* pos_exp, double* window, int st1ID)
{
    if(tracklet.stationID != nStations-1)
    {
        for(int i = 0; i < 3; i++)
...
void KalmanFastTracking_OLD::printAtDetectorBack(int stationID, std::string outputFileName)
{
    TCanvas c1;
...
SRecTrack KalmanFastTracking_OLD::processOneTracklet(Tracklet& tracklet)
{
    //tracklet.print();
    KalmanTrack kmtrk;
...
bool KalmanFastTracking_OLD::fitTrack(KalmanTrack& kmtrk)
{
...
void KalmanFastTracking_OLD::resolveLeftRight(KalmanTrack& kmtrk)
{
    bool isUpdated = false;
...
void KalmanFastTracking_OLD::printTimers() {
	std::cout <<"KalmanFastTracking_OLD::printTimers: " << std::endl;
	std::cout <<"================================================================" << std::endl;
...
void KalmanFastTracking_OLD::chi2fit(int n, double x[], double y[], double& a, double& b)
{
    double sum = 0.;
    double sx = 0.;
    double sy = 0.;
